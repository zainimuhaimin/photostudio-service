  ...
  <div class="flex-none w-full max-w-full px-3">
    <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
      <div class="space-x-4 justify-between flex p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
        <h6 class="dark:text-white">Tabel Jasa</h6>
        <a class="dark:text-white bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" href="<?= base_url() ?>jasa-add">
          Tambah Jasa </a>
      </div>
          ...
          <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
            <thead class="align-bottom">
              <tr>
                ...
                <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Harga Jasa</th>
              ...
              <?php if (!empty($jasas)): ?>
                ...
                foreach ($jasas as $val): ?>
                  <tr>
                    ...
                    <td class="p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent">
                      <p id="hargajasa" class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80"><?= $val['harga_jasa'] ?></p>
                    </td>
                    <td class="p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent">
                    ...
                    <td class="p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent">
                      <div class="flex space-x-2 ">
                        <a id="edit-pelanggan" class="text-blue-500 hover:text-blue-700" href="<?= base_url() ?>jasa-edit/<?= $val['id_jasa'] ?>">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                          </svg>
                        </a>
                        <button onclick="deleteConfirmation(`Apakah Kamu Ingin Menghapus Data Ini ?`, <?= $val['id_jasa'] ?>)" id="delete-pelanggan" href="" class="text-red-500 hover:text-red-700">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
<script>
  ...
  function deleteConfirmation(message, id) {
    ...
    //validate alat jika data di gunakan dalam transaksi tidak bisa di hapus
  ...
  function getTransactionData(id) {
    $.ajax({
      url: `${baseUrl}jasa/pelanggan-get-transaction-data/${id}`,
      type: 'GET',
      success: function(response) {
        if (response.status === 200) {
          Swal.fire({
            title: 'Peringatan!',
            text: 'Data Ini Tidak Bisa Dihapus Karena Digunakan Dalam Transaksi',
            icon: 'warning',
            confirmButtonText: 'OK',
            customClass: {
              confirmButton: 'bg-blue text-black rounded px-4 py-2 hover:bg-blue-700'
  ...
  const hargaEls = document.querySelectorAll('#hargajasa');
  hargaEls.forEach(el => {
