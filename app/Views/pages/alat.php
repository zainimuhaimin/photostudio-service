              ...
              <?php if (!empty($alats)): ?>
                ...
                foreach ($alats as $val): ?>
                  <tr>
                    ...
                    <td class="p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent">
                      <p id="hargasewa" class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80"><?= $val['harga_alat'] ?></p>
                    </td>
                    <td class="p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent">
                    ...
                    <td class="p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent">
                      <div class="flex space-x-2 ">
                        <a id="edit-pelanggan" class="text-blue-500 hover:text-blue-700" href="<?= base_url() ?>alat-edit/<?= $val['id_alat'] ?>">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                          </svg>
                        </a>
                        <button onclick="deleteConfirmation(`Apakah Kamu Ingin Menghapus Data Ini ?`, <?= $val['id_alat'] ?>)" id="delete-pelanggan" href="" class="text-red-500 hover:text-red-700">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
<script>
  ...
  function getTransactionData(id) {
    $.ajax({
      url: `${baseUrl}alat/pelanggan-get-transaction-data/${id}`,
      type: 'GET',
      success: function(response) {
        if (response.status === 200) {
          Swal.fire({
  ...
  // document.getElementById('hargasewa').innerText = <?= $val['harga_alat'] ?> ? `Rp ${parseInt(<?= $val['harga_alat'] ?>).toLocaleString('id-ID')}` : '';
  
  const hargaEls = document.querySelectorAll('#hargasewa');
  hargaEls.forEach(el => {
