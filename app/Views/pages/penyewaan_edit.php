  ...
  <div class="flex-none w-full max-w-full px-3">
    <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
      <div class="space-x-4 justify-between flex p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
        <h6 class="dark:text-white">Penyewaan Edit Alat</h6>
      </div>
      <div class="flex px-0 pt-0 pb-2">
        <div class="p-6 overflow-x-auto w-1/2">
          <form role="form text-left" action="<?= base_url() ?>penyewaan-update" method="POST">
            <div class="mb-4">
              <input type="hidden" name="id_penyewaan" value="<?= $penyewaan['id_penyewaan'] ?>">
              <input type="hidden" name="id_pembayaran" value="<?= $penyewaan['id_pembayaran'] ?>">
              ...
              <select id="alat" name="alat" class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                <?php foreach ($alats as $val): ?>
                  <option <?= $val['id_alat'] == $penyewaan['id_alat'] ? 'selected' : ''  ?> value="<?= $val['id_alat'] ?>" data-harga="<?= $val['harga_alat'] ?>"><?= $val['nama_alat'] ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="mb-4">
              <label for="jadwal">Pilih Jadwal:</label>
              <input required value="<?= $penyewaan['tanggal'] ?>" placeholder="Pilih jadwal ..." type="text" id="jadwal" name="jadwal" class="block appearance-none bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline" />
            </div>
            <div class="mb-4">
              ...
              <select name="pembayaran-metode" class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                ...
                <option value="CASH" <?= $penyewaan['metode_pembayaran'] == 'CASH' ? 'selected' : '' ?>>CASH</option>
              </select>
            </div>
            <div class="mb-4">
...
<script>
  ...
  document.getElementById('alat').addEventListener('change', function() {
    ...
    document.getElementById("jadwal")._flatpickr.clear();
    updatePrice();
